...
public class Hard : MonoBehaviour
{
    ...
    {
        // Ensure GameOver scene shows the final (penalized) score
    ...
    // --- Always deduct 1 point on any mistake (never below 0) ---
    ...
    private void ApplyMistakePenalty()
    {
        ...
        if (score < oldScore)
        {
            // Optional: annotate the current result line
            ResultText.text += "  (-1 point)";
        }
    }
    ...
    private void CountAsMiss()
    {
        ...
        // Deduct 1 point for this mistake
        ApplyMistakePenalty();
    ...
    private void Check(int spoken)
    {
        ...
        {
            // Wrong answer: lose life and 1 point
            lives = Mathf.Max(0, lives - 1);
            RunStats.UseLife();
